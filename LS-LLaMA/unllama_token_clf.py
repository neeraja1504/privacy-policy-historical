...
# # Ensure CPUAdamBuilder is correctly initialized
# deepspeed.ops.adam.cpu_adam.CPUAdamBuilder().load()

os.environ['HUGGINGFACEHUB_API_TOKEN'] =  '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'
os.environ['HF_HOME'] = 'scratch/bbzy/neeraja1504/cache'
os.environ['TRANSFORMERS_CACHE'] = 'scratch/bbzy/neeraja1504/cache'
...
def load_CI():
    ...
    for split_name in ['train_large', 'test_large']:
        ...
        ret[split_name] = Dataset.from_pandas(df)
#         ret[split_name] = Dataset.from_list(data)
    for split in ret:
        ret[split] = ret[split].shuffle()
    return DatasetDict(ret)
...
if task == 'wnut_17':
    ...
    ds = load_CI()
    label2id = {'O': 0, 'B-TP': 1, 'B-ATTRIBUTE': 2, 'B-SENDER': 3, 'B-RECEIVER': 4, 'B-SUBJECT': 5}
else:
    raise NotImplementedError
...
model = UnmaskingLlamaForTokenClassification.from_pretrained(
    model_id, num_labels=len(label2id), id2label=id2label, label2id=label2id, token=os.environ["HUGGINGFACEHUB_API_TOKEN"]
).bfloat16()
...
trainer = Trainer(
    model=model,
    args=training_args,
    train_dataset=tokenized_ds["train_large"],
    eval_dataset=tokenized_ds["test_large"],
    tokenizer=tokenizer,
